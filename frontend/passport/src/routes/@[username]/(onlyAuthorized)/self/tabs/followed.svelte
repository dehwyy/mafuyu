...
{#if atLeastOne}
  <div in:fade={{ duration: 100 }} class="flex flex-col gap-y-5 py-5">
    {#each $followers as followed}
      {#if followed.data}
        <a in:fade={{ duration: 100 }} href={CreateNavigation.ToUser(followed.data.username)}>
          <UserPanel username={followed.data.username} pseudonym={followed.data.pseudonym} picture={followed.data.picture} />
        </a>
      {/if}
    {/each}
  </div>
