<script lang="ts">
  ...
  import Datepicker from '$lib/components/form/items/datepicker.svelte'
  import InputWithLabel from '$lib/components/form/items/input.svelte'
  import { DevFallbackImages } from '$lib/const'
  import { useEditUser, useUserInfo } from '$lib/query/user'
  ...
  $: has_changes =
    ...
    bio !== initialBio ||
    JSON.stringify(selected_languages) !==
      JSON.stringify(initialSelectedLanguages)
...
<section class="col-span-2 flex flex-col gap-y-5 settings px-5">
  <div
    class={`${
      has_changes ? 'bottom-5' : '-bottom-20'
    } fixed self-end  z-10 -mr-6 transition-all duration-200`}
  >
    <button
  ...
  <div
    class={`${
      has_changes ? 'bottom-5' : '-bottom-20'
    } fixed self-end z-10 mr-10 transition-all duration-200 delay-100`}
  >
    <button
      on:click={() => DiscardAll()}
      class="w-[50px] h-[50px] variant-glass-error btn rounded-full px-3 border border-error-500"
    >
      {@html CloseIconRaw}
    </button>
  </div>
  ...
  <hr />
  <article
    class:absolute={false}
    class="flex items-center gap-x-10 opacity-10 line-through select-none"
  >
    <p>Birthday</p>
    <button
      on:click|stopPropagation={() => {}}
      class="btn variant-glass-primary max-w-full w-[200px]">Pick</button
    >
