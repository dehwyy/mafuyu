...
<main class="grid place-items-center min-h-[80vh] mt-10">
  <form class="max-w-[400px] w-full card p-10">
    <h3 class="h3 text-center mb-10">Sign in to <span class="text-primary-400 underline">Mafuyu</span></h3>
    <input placeholder='email@example.com' type="email"  bind:value={$email.value} class={`input`} />
    <hr class="my-3"/>
    <input placeholder="password" type="password"  minlength="8" bind:value={$password.value} class="input mb-2" />
