...
>
  <button
    class="p-5 flex gap-x-2"
    class:cursor-default={!openable}
    on:click={() => openable && (isOpen = !isOpen)}
  >
    <svg
      height="24px"
      viewBox="0 0 24 24"
    ...
    {#if openable}
      <div
        class={`${isOpen ? 'rotate-180' : 'rotate-0'} icon-sm transition-all`}
      >
        {@html ShevronIconRaw}
      </div>
    {/if}
  ...
  <div class="">
    {#if openable}
      <div
        ...
        class="absolute top-[33px] left-[57.5px] z-10 transition-all"
      >
        <svg
          height={lineHeight}
          width="24px"
      ...
      <div
        ...
        class={`${
          isOpen ? '' : '!max-h-[0px]'
        } px-10 flex flex-col gap-y-5 transition-all duration-500 ease-in-out overflow-hidden`}
      >
        {#each nodes as node}
