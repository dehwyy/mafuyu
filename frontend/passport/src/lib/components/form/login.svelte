<script lang="ts">
  ...
  const popup_options = [
    {
      icon: ResetPasswordIconRaw,
      text: 'reset password',
      action: () => toast_store.trigger(toast)
    }
  ]
...
<form
  ...
  method="POST"
>
  <h3 class="h2 sm:h3 text-center mb-10">
    Sign in to <span class="text-primary-400 underline">Mafuyu</span>
  </h3>
  <input
  ...
  <div class="w-fit ml-auto pr-5">
    <OptionsPopup
      unique_target_name="forgot-password-popup"
      options={popup_options}
    >
      <button
        on:click|preventDefault
        class="hover:text-primary-400 underline transition-all"
      >
        Forgot password?
      </button>
    </OptionsPopup>
  ...
  <div class="w-2/3 mx-auto">
    <button
      disabled={!$f.valid}
      type="submit"
      class="variant-filled-primary btn w-full my-4">Login</button
    >
  </div>
  <div class="">
    <p class="text-center">
