...
<div
  on:click|stopPropagation={() => (is_focused = true)}
  aria-hidden="true"
  class="w-full writer overflow-hidden focus-visible:!outline-none"
>
  <Textarea
    bind:text
    {placeholder}
  ...
  <div
    class={`${
      is_focused || text || images.length ? 'max-h-[100px]' : 'max-h-0'
  ...
  >
    ...
    <div class="w-full flex items-center pl-8 pr-6 pb-4">
      ...
      <button
        ...
        class="text-sm ml-auto block btn variant-filled-surface select-none"
        >{button_text}</button
      >
    </div>
  </div>
