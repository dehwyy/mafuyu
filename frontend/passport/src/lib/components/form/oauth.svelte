...
<div class="relative my-7">
  <div
    class="absolute -top-2.5 left-1/2 -translate-x-1/2 text-sm bg-surface-800 px-3"
  >
    or
  </div>
  <hr />
...
<div class="flex flex-col gap-y-3">
  <button
    on:click={() => $oauth2Redirect.mutate({ provider: 'google' })}
    class="oauth-button"
  >
  ...
  <button
    on:click={() => $oauth2Redirect.mutate({ provider: 'github' })}
    class="oauth-button"
  >
    <span class="oauth-icon">
      {@html GithubIconRaw}
