<script lang="ts">
  ...
  import { useUserInfo } from "$lib/queries/user"
  ...
  $: user = $authed_user_store ? useUserInfo($authed_user_store.username) : null
</script>

<button class="h-[40px]" use:popup={settingsClick}>
  <div class="max-h-full max-w-[40px] overflow-hidden rounded-full border border-surface-500">
