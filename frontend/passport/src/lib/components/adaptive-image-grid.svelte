...
<section class="w-full px-5 py-2 grid grid-flow-col-dense gap-2"
         style={`max-height: ${max_height};`}>
  {#each images as image, i}
    <div class="overflow-hidden relative">
      {#if with_delete} <button on:click={e => RemoveImage(e, i)} class="absolute font-[500] right-1 -top-1 text-2xl cursor-pointer hover:text-red-300 transition-all">x</button>{/if}
