<script lang="ts">
  ...
  const [currentUser, currentUserStore] = useUserInfo({ oneofKind: "userId", userId: $authed_user_store?.id })
  $: currentUserStore.set({ getBy: { oneofKind: "userId", userId: $authed_user_store?.id } })

  $: isCurrentUser = username === $authed_user_store?.username
