<script lang="ts">
  ...
  const OnFileSelect = async (e: Event) => {
    ...
    const loaded_images: Array<Image | undefined> = new Array(files_count).fill(undefined) // base64[]
    for (let i = 0; i < files_count; i++) {
      const image_to_load = input.files.item(i)! // should exist
      console.log(image_to_load)
      // image_to_load.size > 10 * 8 // TODO: add size

      if (["image/jpeg", "image/png", "image/webp", "image/gif"].includes(image_to_load.type)) {
    ...
    if (loaded_images.length + processed_images.length <= MaxImages) {
      unprocessed_images = current_image ? [...unprocessed_images, current_image, ...images] : [...unprocessed_images, ...images]
  ...
  $: OnSaveImage = (type_callback: (type: string) => string) => {
    ...
    SaveImage({ data: type_callback(current_image.type), type: current_image.type })
  }
</script>
