<script lang="ts">
  ...
  import useUserProfileActions, {
    UserStatus
  } from '$lib/hooks/use-user-profile-actions'
...
<div
  class={`${
    $status === UserStatus.None ? '!max-h-[0px]' : 'max-h-[30px]'
  } transition-all grid duration-500 mb-5 text-center text-lg`}
>
...
<button
  ...
  class="btn hover:variant-soft w-full font-bold text-lg border border-transparent my-2"
  >Actions</button
>
<div
  data-popup="user_actions"
  class="card border border-surface-600 z-10"
>
